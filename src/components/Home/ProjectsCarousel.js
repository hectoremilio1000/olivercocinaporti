      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between mb-10 gap-6">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-widest text-[var(--oliver-blue)] mb-3">Casos reales</p>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--oliver-navy)]">Proyectos entregados</h2>
            <p className="mt-4 text-lg text-[var(--neutral-700)]">Una muestra de cocinas que hemos diseñado y montado en restaurantes, hoteles y comedores.</p>
          </div>
          <div className="flex items-center gap-3" role="group" aria-label="Controles del carrusel">
            <button onClick={() => scroll(-1)} onKeyDown={onKeyNav(-1)} aria-label="Proyecto anterior" className="w-11 h-11 rounded-full bg-white border border-[var(--neutral-200)] flex items-center justify-center hover:bg-[var(--oliver-navy)] hover:text-white hover:border-[var(--oliver-navy)] transition-colors">
              <ChevronLeft size={20} />
            </button>
            <button onClick={() => scroll(1)} onKeyDown={onKeyNav(1)} aria-label="Proyecto siguiente" className="w-11 h-11 rounded-full bg-white border border-[var(--neutral-200)] flex items-center justify-center hover:bg-[var(--oliver-navy)] hover:text-white hover:border-[var(--oliver-navy)] transition-colors">
          ...
          {PROJECTS.map((p) => (
            <Link key={p.id} href={`/proyectos/${p.id}`} className="group flex-shrink-0 w-[85%] sm:w-[60%] lg:w-[32%] snap-start bg-white rounded-lg overflow-hidden border border-[var(--neutral-200)] hover:shadow-xl transition-shadow">
              ...
              <div className="p-6">
                ...
                <span className="inline-flex items-center gap-1 text-sm font-semibold text-[var(--oliver-blue)] group-hover:gap-2 transition-all">Ver caso <ArrowUpRight size={16} /></span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="flex justify-center mt-10">
          <CTAButton href="/proyectos" variant="primary" size="md">Ver todos los proyectos</CTAButton>
        </div>
      </div>
    </section>
  );
